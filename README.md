...
When processing time is greater than one second (or ACK_TIMEOUT/2), the last message from the initiator is confirmed with a code 2.02 (accepted), and it is up to the initiator to poll the responder periodically for data. If not ready, the responder must answer with a code 2.02 (accepted), until it is ready. Then, it answers with a code 2.00 (ok) when no more content should be transferred or with a code 2.06 (continue).
